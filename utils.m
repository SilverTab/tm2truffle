...
#define begintry @try {
#define endtry	} @catch (NSException *innerexp) { NSLog(@"Exception in %s : %d :: %@", __FILE__, __LINE__, innerexp); }
...
{
	NSDictionary *mainMenu = [infoPlist objectForKey:@"mainMenu"];
	NSArray *items = [mainMenu objectForKey:@"items"];
	...
	if ([infoPlist objectForKey:@"deleted"])
		[excluded addObjectsFromArray:[infoPlist objectForKey:@"deleted"]];
	if ([mainMenu objectForKey:@"excludedItems"])
		[excluded addObjectsFromArray:[mainMenu objectForKey:@"excludedItems"]];
	
	__block BOOL (^parseMenu)(NSString *, NSArray *, id);
		
	parseMenu = [^ BOOL (NSString *name, NSArray *items, id selfmlnode){
		
		if ([name length])
			[selfmlnode addChild:name];
		...
		for (NSString *uuid in items)
		{
			...
			NSString *ident = [uuidIdentifierMap objectForKey:uuid];
			if ([ident length])
			{
				[selfmlnode addChild:ident];
				lastWasSep = NO;
				addedNonSep = YES;
				continue;
			}
			...
			// Is this a submenu?
			NSDictionary *submenu = [submenus objectForKey:uuid];
			if (submenu)
			{
				id submenuNode = SELFML(@"submenu");
				NSString *submenuName = [submenu objectForKey:@"name"];
				if (![submenuName length])
					submenuName = @"Actions";
				
				if (parseMenu(submenuName, [submenu objectForKey:@"items"], submenuNode))
				{
					[selfmlnode addChild:submenuNode];
					addedNonSep = YES;
				}
			}
		}
	...
	id menu = SELFML(@"menu");
	if (parseMenu(nil, items, menu))
		[[menu selfmlRepresentation] writeToFile:outputPath atomically:YES encoding:NSUTF8StringEncoding error:nil];
}

int importMetaData(NSString *bundleRoot, NSString *outputFile)
{
	uuidIdentifierMap = [[NSMutableDictionary alloc] init];
	...
	NSString *menuOutputPath = [outputFile stringByAppendingPathComponent:@"menu.selfml"];
	...
	if ([displayName length]) {
		...
		[rootNode addChild:displayNameNode];
	}
	
	// name
	if ([[metaDic objectForKey:@"contactName"] length]) {
...
void importMetaData_menus(NSString *bundleRoot, NSString *outputFile)
{
	NSFileManager *fm = [NSFileManager defaultManager];
	NSString *metaDataPath = [bundleRoot stringByAppendingPathComponent:@"info.plist"];
	NSString *menuOutputPath = [outputFile stringByAppendingPathComponent:@"menu.selfml"];
	if (![fm fileExistsAtPath:metaDataPath]) {
		NSLog(@"This is not normal...No info.plist, can't import menus! DUCK AND COVER!");
		return;
	}
...
void processSnippet(NSString *snippetPath, NSString *outputPath)
{
	begintry
		...
		// output to disk...
		[uuidIdentifierMap setValue:[NSString stringWithFormat:@"snippet.%@", [newName stringByDeletingPathExtension]] forKey:[snippetAsDic objectForKey:@"uuid"]];
...
[commandDicInput rangeOfString:@"TM_SUPPORT_PATH"].location != NSNotFound || [commandDicInput rangeOfString:@"TM_BUNDLE_SUPPORT"].location != NSNotFound))
	...
	*/
	...
	[uuidIdentifierMap setValue:[NSString stringWithFormat:@"command.%@", [fileName stringByDeletingPathExtension]] forKey:[commandDic objectForKey:@"uuid"]];
